# python3 ./scripts/mobilenet_baseline_evaluate.py
import json
import os
import torch
...
# weights from our training are stored here
...
# rebuild model using the learned weights
...
for actual_class in [FIRE_CLASS, "non_fire"]:
    ...
    for filename in os.listdir(class_dir):
        ...
        # open and load image in RGB channel
        filepath = os.path.join(class_dir, filename)
        image = Image.open(filepath).convert("RGB")
        input_tensor = transform(image).unsqueeze(0).to(device)

        # feed image to model, and get its predictions
        with torch.no_grad():
